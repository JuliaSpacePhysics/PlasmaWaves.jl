...
electrostatic dispersion for isotropic Maxwellian species.
...
# References
- [BO](https://github.com/hsxie/bo)
- [plasmapy.dispersion](https://docs.plasmapy.org/en/latest/dispersion/)
```
"""
module Dispersion
using LinearAlgebra
...
"""
function cold_em_root(
        k::Float64, θ::Float64, B0::Float64, species::Vector{Species};
        branch::Int = +1, init_ω::Union{Nothing, Float64} = nothing,
        tol::Float64 = 1.0e-6, max_iter::Int = 50
    )
    θrad = θ
    # initial guess: vacuum dispersion unless provided
    ω = isnothing(init_ω) ? max(c * k, 1.0e-3) : init_ω
    for _ in 1:max_iter
...
"""
function cold_em_polarization(
        ω::Float64, k::Float64, θ::Float64,
...
"""
function kinetic_es_root(
        k::Float64, species::Vector{Species};
        init_ω::Union{Nothing, Complex} = nothing,
        tol::Float64 = 1.0e-8, max_iter::Int = 50
    )
    if isnothing(init_ω)
